// nm-app.jsx — shell: routing, fleet/role access, nav, persistence, tweaks
...
function App() {
  ...
  const [user, setUser] = useStateA(() => LS.get("nm_user", null));
  const [fleet, setFleet] = useStateA("ALL");
  const [route, setRoute] = useStateA({ name: "dashboard", param: null });
  const [tab, setTab] = useStateA("dashboard");
  const [toast, setToast] = useStateA("");

  const [fleets, setFleets] = useStateA(() => LS.get("nm_fleets", window.FLEETS));
  const [trucks, setTrucks] = useStateA(() => LS.get("nm_trucks", window.SEED_TRUCKS));
  const [issues, setIssues] = useStateA(() => LS.get("nm_issues", window.SEED_ISSUES));
  const [parts, setParts] = useStateA(() => LS.get("nm_parts", window.SEED_PARTS));
  const [inspections, setInspections] = useStateA(() => LS.get("nm_inspections", window.SEED_INSPECTIONS));
  const [invoices, setInvoices] = useStateA(() => LS.get("nm_invoices", window.SEED_INVOICES));
  const [usage, setUsage] = useStateA(() => LS.get("nm_usage", window.SEED_USAGE));
  const [history, setHistory] = useStateA(() => LS.get("nm_history", window.SEED_HISTORY));

  // keep the global fleet registry in sync so every screen sees admin-created fleets
  window.FLEETS = fleets;

  useEffectA(() => LS.set("nm_user", user), [user]);
  useEffectA(() => LS.set("nm_fleets", fleets), [fleets]);
  useEffectA(() => LS.set("nm_trucks", trucks), [trucks]);
  useEffectA(() => LS.set("nm_issues", issues), [issues]);
  useEffectA(() => LS.set("nm_parts", parts), [parts]);
  useEffectA(() => LS.set("nm_inspections", inspections), [inspections]);
  useEffectA(() => LS.set("nm_invoices", invoices), [invoices]);
  useEffectA(() => LS.set("nm_usage", usage), [usage]);
  useEffectA(() => LS.set("nm_history", history), [history]);

  // fleets this user is allowed to see
  ...
  const vTrucks = useMemoA(() => scope(trucks), [trucks, fleet, user, fleets]);
  const vIssues = useMemoA(() => {
    const ids = new Set(vTrucks.map((x) => x.id));
    return issues.filter((i) => ids.has(i.truckId));
  }, [issues, vTrucks]);
  const vParts = useMemoA(() => scope(parts), [parts, fleet, user, fleets]);
  const vInspections = useMemoA(() => scope(inspections), [inspections, fleet, user, fleets]);
  const vInvoices = useMemoA(() => scope(invoices), [invoices, fleet, user, fleets]);
  ...
  const saveCheck = (truckId, payload) => {
    ...
    setInspections((arr) => [{
      id: "r" + Date.now(), truckId, fleet: tr.fleet, date: today, by: user.name,
      attn, missing: payload.missing || "", general: payload.general || "",
      results: payload.results || {}, notes: payload.notes || {},
    }, ...arr]);
  ...
  const updateTruckDocs = (truckId, fields) => {
    setTrucks((arr) => arr.map((tr) => tr.id === truckId ? { ...tr, ...fields } : tr));
    showToast("Documents updated");
    go("truck", truckId);
  };
  const addService = (truckId, rec) => {
    setHistory((arr) => [{ id: "h" + Date.now(), truckId, by: user.name, ...rec }, ...arr]);
  ...
  const Tweaks = (
    <TweaksPanel>
      ...
      <TweakRadio label="Theme" value={t.theme} options={["navy", "midnight", "hivis"]}
        onChange={(v) => setTweak("theme", v)} />
      <TweakColor label="Accent (orange)" value={t.accent2} options={window.ACCENT2_OPTIONS}
        onChange={(v) => setTweak("accent2", v)} />
      <TweakSection label="Typography" />
      <TweakRadio label="Font" value={t.font} options={["jakarta", "plex", "system"]}
  ...
  switch (route.name) {
    ...
    case "truck": {
      const tr = trucks.find((x) => x.id === route.param);
      screen = tr ? <TruckDetail truck={tr} issues={issues} usage={usage} parts={parts} history={history} go={go} onToggleOOS={toggleOOS} canEdit={canEdit} /> : <Trucks fleet={fleet} multiFleet={multiFleet} fleetIds={myFleets} trucks={vTrucks} go={go} />;
      break;
    }
  ...
  const cycleFleet = () => {
    const opts = ["ALL", ...myFleets];
    const idx = opts.indexOf(fleet);
    setFleet(opts[(idx + 1) % opts.length]);
  };
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {multiFleet && fleet !== "ALL" && (
            <span style={{ display: "flex", alignItems: "center", gap: 5, border: `1px solid ${C.border}`, background: C.surface2, borderRadius: 999, padding: "6px 11px", color: C.mutedFg, whiteSpace: "nowrap" }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 800, color: C.fg }}>{window.FLEETS[fleet]?.name || fleet}</span>
            </span>
          )}
