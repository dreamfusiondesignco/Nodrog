// nm-screens-core.jsx — Login, Dashboard, Trucks, TruckDetail
const { useState: useStateC, useMemo: useMemoC } = React;

function Logo({ size = 44, light }) {
  const R = (typeof window !== "undefined" && window.__resources) || {};
  const src = light ? (R.markLight || "assets/nodrog-mark-light.svg") : (R.markNavy || "assets/nodrog-mark.svg");
  return (
...
function Login({ onLogin }) {
  const [email, setEmail] = useStateC("marlon@nodrog.com");
  const [pin, setPin] = useStateC("");
  const [err, setErr] = useStateC("");
  const submit = () => {
    const u = window.SEED_USERS.find((x) => x.email === email.trim().toLowerCase() && x.pin === pin.trim());
...
function FleetChip({ fleet }) {
  const color = fleet === "IGL" ? C.accent : fleet === "MASSY" ? C.accent2 : C.mutedFg;
  const label = fleet === "SHARED" ? "Shared" : window.FLEETS[fleet]?.name || fleet;
  return <Badge color={color}>{label}</Badge>;
}

function Dashboard({ user, fleet, trucks, issues, parts, go }) {
  ...
  // upcoming reminders: cert expiries + service due within window
  const reminders = [];
  trucks.forEach((t) => {
  ...
  return (
    <div>
      <Header title={`Good day, ${user.name.split(" ")[0]}`} sub={`${fleet === "ALL" ? "All fleets" : window.FLEETS[fleet].full} · Week of ${fmtDate("2026-06-08")}`} />
...
function Trucks({ fleet, multiFleet, fleetIds = ["IGL", "MASSY"], trucks, go }) {
...
function MeterRow({ label, value, max, unit, sub }) {
...
function TruckDetail({ truck, issues, usage, parts, history, go, onToggleOOS, canEdit }) {
  const [tab, setTab] = useStateC("service");
  ...
  return (
    <div>
      <Header title={truck.plate} sub={`${truck.model} · ${truck.fleet === "SHARED" ? "" : window.FLEETS[truck.fleet].name}`} onBack={() => go("trucks")}
        action={<Badge color={statusColor(truck.status)} solid={truck.status === "oos"}>{statusLabel(truck.status)}</Badge>} />
      <div style={{ padding: "0 16px", display: "flex", flexDirection: "column", gap: 12 }}>
        {/* identity card */}
        <div style={{ ...cardStyle(), padding: 0, overflow: "hidden" }}>
          <image-slot id={"truck-photo-" + truck.id} shape="rect" placeholder="Drop or tap to add truck photo"
            style={{ display: "block", width: "100%", height: 150, borderBottom: `1px solid ${C.border}` }}></image-slot>
        ...
        {tab === "issues" && <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {tIssues.map((i) => <IssueCard key={i.id} issue={i} compact />)}
...
Object.assign(window, { Logo, Login, Dashboard, Trucks, TruckDetail, FleetChip, Chip, EmptyNote, MeterRow });
