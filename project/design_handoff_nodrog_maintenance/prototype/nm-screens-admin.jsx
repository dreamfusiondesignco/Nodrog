// nm-screens-admin.jsx — More hub, create part, fleets, weekly reports review, invoices
const { useState: useStateAd } = React;

const money = (n) => "$" + Number(n || 0).toLocaleString("en-US");

// ───────────────────────── More hub ─────────────────────────
function MoreHub({ user, fleet, onFleet, fleetIds = [], multiFleet, fleetCount, inspections, invoices, go }) {
        ...
        {multiFleet && (
          <div style={{ ...cardStyle(), padding: 14 }}>
            ...
            <select value={fleet} onChange={(e) => onFleet(e.target.value)} style={{ width: "100%", minHeight: 48, borderRadius: 11, border: `1px solid ${C.border}`, padding: "0 12px", fontSize: 16, background: C.surface, color: C.fg, boxSizing: "border-box", fontWeight: 700 }}>
              ...
              {fleetIds.map((k) => <option key={k} value={k}>{window.FLEETS[k]?.full || window.FLEETS[k]?.name || k}</option>)}
            </select>
          </div>
        )}
        <div style={{ height: 8 }} />
...
// ───────────────────────── Create part ─────────────────────────
function NewPart({ fleetIds, onSave, go }) {
  const [name, setName] = useStateAd("");
  const [sku, setSku] = useStateAd("");
  const [qty, setQty] = useStateAd("");
  const [min, setMin] = useStateAd("");
  return (
    <div>
...
// ───────────────────────── Manage fleets ─────────────────────────
function ManageFleets({ fleets, trucks, parts, onAdd, go }) {
  const [name, setName] = useStateAd("");
  const [full, setFull] = useStateAd("");
...
function WeeklyReports({ inspections, trucks, go }) {
  const [f, setF] = useStateAd("all");
  const plate = (id) => trucks.find((t) => t.id === id)?.plate || "—";
  const model = (id) => trucks.find((t) => t.id === id)?.model || "";
...
function ReportDetail({ report, trucks, go }) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: "0 16px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <div style={{ fontSize: 11.5, color: C.mutedFg, padding: "2px 4px" }}>Showing recorded line items. A live build keeps every line of the sheet.</div>
        <div style={{ height: 8 }} />
      </div>
    </div>
  );
...
// ───────────────────────── Invoices ─────────────────────────
const invTotal = (inv) => (inv.items || []).reduce((s, it) => s + (+it.qty || 0) * (+it.unit || 0), 0);
const invStatusColor = (s) => s === "paid" ? C.ok : s === "sent" ? C.accent : C.mutedFg;

function Invoices({ invoices, trucks, go }) {
  const [f, setF] = useStateAd("all");
  const plate = (id) => trucks.find((t) => t.id === id)?.plate || "—";
...
function InvoiceDetail({ invoice, trucks, onStatus, go }) {
    ...
    <div>
      ...
      <div style={{ padding: "0 16px 20px", display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ ...cardStyle(), padding: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[["Party", invoice.party], ["Fleet", window.FLEETS[invoice.fleet]?.name || invoice.fleet], ["Truck", truck?.plate || "—"], ["Due", fmtDate(invoice.dueDate)]].map(([k, v]) => (
              <div key={k}><div style={{ fontSize: 11, color: C.mutedFg, marginBottom: 3 }}>{k}</div><div style={{ fontSize: 14, fontWeight: 700, color: C.fg }}>{v}</div></div>
            ))}
          </div>
        </div>
...
function NewInvoice({ fleetIds, trucks, onSave, go }) {
  const [fleet, setFleet] = useStateAd(fleetIds[0] || "IGL");
  const [truckId, setTruckId] = useStateAd("");
  const [party, setParty] = useStateAd("");
  const [kind, setKind] = useStateAd("Payable");
  const [due, setDue] = useStateAd("");
  const [notes, setNotes] = useStateAd("");
  const [items, setItems] = useStateAd([{ desc: "", qty: "1", unit: "" }]);
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <div style={{ flex: 1 }}>
            <Select label="Fleet" value={fleet} onChange={(e) => { setFleet(e.target.value); setTruckId(""); }}>
              {fleetIds.map((k) => <option key={k} value={k}>{window.FLEETS[k]?.name || k}</option>)}
            </Select>
          </div>
          <div style={{ flex: 1 }}>
...
// ───────────────────────── Record part used on a truck ─────────────────────────
function NewUsage({ truck, parts, onSave, go }) {
  const avail = parts.filter((p) => p.fleet === "SHARED" || p.fleet === truck.fleet);
  const [partId, setPartId] = useStateAd(avail[0]?.id || "");
  const [qty, setQty] = useStateAd("1");
  const [date, setDate] = useStateAd("2026-06-11");
...
// ───────────────────────── Edit truck documents ─────────────────────────
function EditDocs({ truck, onSave, go }) {
  const [chassis, setChassis] = useStateAd(truck.chassis || "");
  const [insuranceExp, setIns] = useStateAd(truck.insuranceExp || "");
  const [fitnessExp, setFit] = useStateAd(truck.fitnessExp || "");
  const [mvRegExp, setMv] = useStateAd(truck.mvRegExp || "");
  const [carrierLicExp, setCar] = useStateAd(truck.carrierLicExp || "");
  const [fireExtDate, setFire] = useStateAd(truck.fireExtDate || "");
  const D = ({ label, value, onChange }) => (
    <Field label={label} value={value} onChange={onChange} type="date" />
  );
  return (
    <div>
...
function NewService({ truck, onSave, go }) {
  const [date, setDate] = useStateAd("2026-06-11");
  const [type, setType] = useStateAd("");
  const [miles, setMiles] = useStateAd(String(truck.odometer || ""));
  const [notes, setNotes] = useStateAd("");
...
Object.assign(window, { MoreHub, NewPart, ManageFleets, WeeklyReports, ReportDetail, Invoices, InvoiceDetail, NewInvoice, NewUsage, EditDocs, NewService, money, invTotal });
